...
\begin{abstract}
This paper presents Yet Another Audio analyzeR (YAAR), a command-line and interactive plotting tool for real-time audio analysis. YAAR combines time-domain monitoring, FFT-based spectral estimation, rolling magnitude averaging, automatic tone detection, and measurement of total harmonic distortion (THD), total harmonic distortion plus noise (THD+N), intermodulation distortion (IMD), signal-to-noise ratio (SNR), signal-to-noise-and-distortion ratio (SINAD), effective number of bits (ENOB), and electrical output metrics such as $V_{\mathrm{RMS}}$ and $P_{\mathrm{RMS}}$. The tool supports both live acquisition from an audio backend and fully synthetic signal generation for repeatable test scenarios. This document serves as a professional user and technical manual, describing the operating principles, command-line interface, measurement workflow, configuration parameters, and practical usage recommendations.
\end{abstract}
...
Audio test workflows often require a compact tool that can acquire signals, display them in real time, and compute practical quality metrics without the overhead of a full laboratory software suite. YAAR addresses this need with a lightweight Python implementation centered on FFT analysis and a clear operational model. The program supports either a physical input stream or a simulation path, which makes it useful both for bench validation and algorithm development.
...
\begin{enumerate}[leftmargin=*,nosep]
    ...
    \item CSV and plot export for repeatable reporting, and
    \item deterministic simulation of noise and distortion scenarios.
\end{enumerate}
